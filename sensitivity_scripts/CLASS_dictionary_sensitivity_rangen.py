...
class Dictionary:
    def __init__(self, ngrams, mincount, bucket, run, sensitivity_file):
        ...
        self.x_rtest, x_rval = self.split_Rtest_Rval(manual_instances)
        self.y_rtest, y_rval = self.split_Rtest_Rval(y_manual)
        
        x_rtest_sens, y_rtest_sens = self.split_sensitivity()        # ***************************************
        
        self.n_rtest = len(x_rtest_sens)
        self.n_rval = len(x_rval)
        print("x_rtest_sens: ", self.n_rtest, " x_rval: ", self.n_rval)
        print("y_rtest_sens: ", len(y_rtest_sens), " y_rval: ", len(y_rval))
        print()
        ...
        self.X_RTEST = self.create_bagngrams(x_rtest_sens)
        self.X_RVAL = self.create_bagngrams(x_rval)
        self.Y_RTEST = self.create_label_vec(self.n_rtest, self.nclasses, y_rtest_sens)
    ...
    def split_sensitivity(self):
        #for filename in os.listdir(self.sensitivity_dir):
            #if '_trial'+str(self.run_number)+'_' in filename:
        subset = np.loadtxt(self.sensitivity_file, dtype=np.object)
                
        subset = subset.astype(int).tolist()  
        x_sub = [self.x_rtest[i] for i in subset]
        y_sub = [self.y_rtest[i] for i in subset]

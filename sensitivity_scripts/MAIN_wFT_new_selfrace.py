...
from CLASS_dictionary_RACE_selfrace import Dictionary
...
def get_args():
    ...
    parser.add_argument('-m', '--sens_dir', action='store', help='Dir', required=True)
...
def create_dictionary(WORDGRAMS, MINCOUNT, BUCKET, run, sensitivity_dir):
    print("starting dictionary creation") 

    # dictionary must be recreated each run to get different subsample each time
    start = time.time()
    dictionary = Dictionary(WORDGRAMS, MINCOUNT, BUCKET, run, sensitivity_dir)
    end = time.time()
...
def main():
    ...
    sensitivity_dir = args['sens_dir']
    
    dictionary = create_dictionary(WORDGRAMS, MINCOUNT, BUCKET, run, sensitivity_dir)

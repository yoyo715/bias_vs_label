...
void FastText::loadModel(const std::string& filename) {
  ...
  if (!ifs.is_open()) {
    std::cerr << "Model file cannot be opened for loading! " << filename << std::endl;
    exit(EXIT_FAILURE);
  }
  if (!checkModel(ifs)) {
...
void FastText::trainThread(int32_t threadId, std::string test_file) {
    ...
    while (tokenCount < args_->epoch * ntokens) {
        ...
        std::cout<<" ThreadID: " << threadId << " progress: " << progress<<std::endl;
        real lr = args_->lr * (1.0 - progress);
        if (args_->model == model_name::sup) {
...
//                 
//                 //check test file
//                 std::string testfile = args_->test;
//                 if (testfile == "-") {
//                     //fasttext.test(std::cin, k);
//                 }
//                 else {
//                     std::ifstream TEST_ifs(testfile);
//                     if (!ifs.is_open()) {
//                         std::cerr << "Test file cannot be opened! " << testfile << std::endl;
//                         exit(EXIT_FAILURE);
//                     }
//                     //fasttext.test(ifs, k);
//                     //std::ifstream TEST_ifs(testfile);
...
void FastText::train(std::shared_ptr<Args> args) {
    args_ = args;
    std::string test_file = args_->test;
    ...
    if (args_->thread > 1) {
        std::vector<std::thread> threads;
        for (int32_t i = 0; i < args_->thread; i++) {
            threads.push_back(std::thread([=]() { trainThread(i, test_file); }));
        }
        for (auto it = threads.begin(); it != threads.end(); ++it) {
            it->join();
